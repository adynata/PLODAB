...
// TO DO: see how x0 position is stored!
...
function update(source) {
    ...
    nodeEnter.append("rect")
        ...
        .style("fill", function (d) {
          var nodeName = d.name.toLowerCase().replace(/\s/g, '_');
          var parentName = "";
          if (d.parent) {
            parentName = d.parent.name.toLowerCase().replace(/\s/g, '_');
          }
          var colorNames = Object.keys(dataSets.colors);
          if (colorNames.contains(nodeName)) {
            return dataSets.colors[nodeName];
          } else if (colorNames.contains(parentName)) {
            // TODO: change color of child nodes here
            return dataSets.colors[parentName];
          } else {
            return "white";
          }
        })
      ...
      .append("xhtml:body")
        ...
        .html(function(d) {
          if (d.level === "mid") {
            ...
            return "<div class='tree-node' style='display: table; width: "+ d.width +"px; height: 100px;'><p class='tree-text' style='color:dark grey;display: table-cell; text-align: center; vertical-align: middle;'>" + d.name + "</p></div>";
          } else if ( d.level === "last") {
            return "<div class='tree-node' style='display: table; width: 100px; height: 50px;'><p class='tree-text' style='color:dark grey; display: table-cell; text-align: center; vertical-align: middle;'>" + d.name + "</p></div>";
          } else {
            return "<div class='tree-node' style='display: table; width: "+ d.width +"px; height: 150px;'><div class='tree-text' style='color:grey;display:table-cell; text-align: center; vertical-align: top;width:70%; float:right'><h3 style='padding:5px;margin:0px'>" + d.name + "</h3><p>"+ d.description+ "</p><p> " + d.publicationDate + "</p><p> " + d.collection + "</p><p> " + d.publisher + "</p><p> " + d.typeOfWork + "</p></div><div class='tree-text' style='color:grey; display:table-cell; text-align: center; vertical-align:top; width:28%; border:1px grey solid; height:" + (d.height - 12 )+ "px; float:left; padding:5px; background-color: " + "white" + ";'>"+"IMAGE THUMB"+"</div></div>";
          }
        });
