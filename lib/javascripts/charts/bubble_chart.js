...
(function(){
  ...
  Bubbles = function(max) {
    ...
    var chart, chartScale, clear, click, collide, collisionPadding, connectEvents, data, force, gravity, height, idValue, jitter, label, margin, maxRadius, minCollisionRadius, mouseout, mouseover, node, rScale, rValue, textValue, tick, totalArea, transformData, update, updateActive, updateLabels, updateNodes, width;
    ...
    tick = function(e) {
      ...
      // TODO: the 50px extra padding is only necessary in angular. Figure out why.
      label.style("left", function(d) {
        return ((margin.left + d.x) - d.dx / 2) + 2 + "px";
      }).style("top", function(d) {
        return ((margin.top + d.y) - d.dy / 2) + 70 + "px";
      });
    ...
    updateLabels = function() {
      ...
      // });
      label.style("font-size", function (d) {
          return Math.max(5, rScale(rValue(d) / 60)) + "px";
        }).style("width", function (d) {
