var Bubbles, displayBubbleChart;
...
(function(){
  ...
  Bubbles = function(max) {
    ...
    var chart, chartScale, clear, click, collide, collisionPadding, connectEvents, data, force, gravity, hashchange, height, idValue, jitter, label, margin, maxRadius, minCollisionRadius, mouseout, mouseover, node, rScale, rValue, selectedKeys, textValue, tick, totalArea, transformData, update, updateActive, updateLabels, updateNodes, width;
    ...
    updateNodes = function() {
      ...
      node.enter()
        ...
        .style("fill", function(d) {
          ...
          var saturated = d3.rgb(d.color).hsl();
          saturated.s = saturated.s * 1.2;
          saturated.l = saturated.l * 0.75;
          if (d.selected) {
    ...
    updateLabels = function() {
      ...
      // });
      label.style("font-size", function (d) {
          return Math.max(8, rScale(rValue(d) / 40)) + "px";
        }).style("width", function (d) {
    ...
    gravity = function(alpha) {
      ...
      return function(d) {
        ...
        // console.log(d3.select(d).classed("bubble-selected"));
        // if (!d.selected) {
        //   d.x += (cx - 400 - d.x) * ax;
        //   d.y += (cy - d.y) * ay;
        // } else {
    ...
    click = function(d) {
      if ( !selectedKeys.includes(idValue(d)) ) {
        selectedKeys.push(idValue(d));
      } else {
        removeKey(selectedKeys, idValue(d));
      }

      var uriComponent = encodeURIComponent(selectedKeys.join("--"));
      location.replace("#" + uriComponent);
    ...
    updateActive = function(id) {
      ...
      if (id.length > 0) {
        return d3.select("#status").html("<h3>The subjects <span class=\"active\">" + id + "</span> is now active</h3>");
      } else {
  ...
  root.plotData = function(selector, data, chartFnc) {
    ...
    console.log("before if", difference);
    // check to see if bubbles fit in total area, and how much over
    if ( difference < 900000) {
      console.log(difference, "over threshold");
      var newMaX = chartFnc.maxRadius() * (difference / 700000);
      console.log(newMaX);
  ...
  $(function() {
    ...
    // create an empty array to store selected chart state
    selectedKeys = [];

    selectedKeys.includes = includes;

    // this function is called once data is loaded
    displayBubbleChart = function(data) {
    ...
    d3.select("#text-select").on("change", function(e) {
      ...
      // key = selectedKeys;
      location.replace("#");
      location.search = encodeURIComponent(selectedKeys);
    });

    // load the data!
  //   return d3.csv("/data/" + text.file, display);
  // });
     var bubbleChart = displayBubbleChart( dataSets.subjects );
