var Timeline = function() {

  var margin = {top: 20, right: 20, bottom: 30, left: 40},
      width = 960 - margin.left - margin.right,
      height = 500 - margin.top - margin.bottom;

  var tlx = d3.scale.ordinal()
      .rangeRoundBands([0, width], 0.1);

  var tly = d3.scale.linear()
      .rangeRound([height, 0]);

  var colorQueue = ["#98abc5", "#8a89a6", "#7b6888", "#6b486b", "#a05d56", "#d0743c", "#ff8c00"];

  var color = function() {
    var rectFill = colorQueue.shift();
    colorQueue.push(rectFill);
    return rectFill;
  };

  // d3.scale.ordinal()
      // .range(["black", "#98abc5", "#8a89a6", "#7b6888", "#6b486b", "#a05d56", "#d0743c", "#ff8c00"]);

  var xAxis = d3.svg.axis()
      .scale(tlx)
      .orient("bottom");

  var yAxis = d3.svg.axis()
      .scale(tly)
      .orient("left")
      .tickFormat(d3.format(".2s"));

  var tip = d3.tip()
      .attr('class', 'd3-tip')
      .offset([-10, 0])
      .html(function(d) {
        return "<div><img class='d3-tip-img' src='lib/images/book.svg'><span style='color:white'>" + d.name.name+ "</span></div>";
      });

  var svg = d3.select("#timeline").append("svg")
      .attr("width", width + margin.left + margin.right)
      .attr("height", height + margin.top + margin.bottom)
    .append("g")
    .attr("fill", "black")
      .attr("transform", "translate(" + margin.left + "," + margin.top + ")");

  svg.call(tip);

  data = dataSets.timeline;

  // color.domain(d3.keys(data[0]).filter(function(key) { return key !== "Year"; }));

  // sets the height of each rect on the bar chart, either dynamically, or, in our case, at a fixed height
  data.forEach(function(d) {
    var bookHeight = 7;
    var y0 = 0;
    d.books = Object.keys(d).map(function(name) {
      if (name != "year") {
        return {name: d[name], y0: y0, y1: y0 += bookHeight};
      }
      else {
        // return a null object if iteratinf over the year
        return {name: d[name], y0: y0, y1: y0};
      }
    });
  });

  // sorts states by Population
  // data.sort(function(a, b) { return b.total - a.total; });

  tlx.domain(data.map(function(d) {
    return d.year; }));

  //sets the upper limit for the domain of the y axis
  tly.domain([0, d3.max(data, function(d) {
    return 100;
    //  return d.total;
   })]);

  svg.append("g")
      .attr("class", "x axis")
      .attr("transform", "translate(0," + height + ")")
      .call(xAxis);

  // svg.append("g")
  //     .attr("class", "y axis")
  //     .call(yAxis)
  //   .append("text")
  //     .attr("transform", "rotate(-90)")
  //     .attr("y", 6)
  //     .attr("dy", ".71em");

  var year = svg.selectAll(".year")
      .data(data)
    .enter().append("g")
      .attr("class", "g")
      //  below appends the item to a certain column
      .attr("transform", function(d) { return "translate(" + tlx(d.year) + ",0)"; });

  year.selectAll("rect")
      .data(function(d) {
        return d.books; })
    .enter().append("rect")
      .attr("width", tlx.rangeBand())
      .attr("y", function(d) {
        return tly(d.y1); })
      // abuts each additional stacked item to most recent item in col
      .attr("height", function(d) { return tly(d.y0) - tly(d.y1); })
      .style("fill", function(d) {
        return color();
      })
      .style("stroke", "white")
      .on('mouseover', tip.show)
      .on('mouseout', tip.hide);

  // var legend = svg.selectAll(".legend")
  //     .data(color.domain().slice().reverse())
  //   .enter().append("g")
  //     .attr("class", "legend")
  //     .attr("transform", function(d, i) { return "translate(0," + i * 20 + ")"; });

  // legend.append("rect")
  //     .attr("x", width - 18)
  //     .attr("width", 18)
  //     .attr("height", 18)
  //     .style("fill", color);
  //
  // legend.append("text")
  //     .attr("x", width - 24)
  //     .attr("y", 9)
  //     .attr("dy", ".35em")
  //     .style("text-anchor", "end")
  //     .text(function(d) { return d; });


};

// Helper function that simplifies the calling
// of our chart with it's data and div selector
// specified
var plotTimelineData = function(selector, data, chartFnc) {
  d3.select(selector + ' svg').remove();
  d3.select(selector + ' div').remove();
  var timeline = d3.select(selector).datum(data).call(chartFnc);
};

// run on document ready
$(function() {
  var key, chartFnc, text;

  // create a new Bubbles chart
  timelineFunc = Timeline();

  // this function is called once data is loaded
  displayTree = function(data) {
    return plotData("#timeline", data, timelineFunc);
  };

  // put the data in!!
 });
